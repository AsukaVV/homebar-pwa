...
function renderTagButtons(container, selectedSet, scope) {
  container.innerHTML = flavors.map((flavor) => {
    ...
    return `<button class="tag-chip${active}" data-scope="${scope}" data-flavor="${flavor.id}" type="button">${flavor.label}</button>`;
  }).join("");
...
async function autoTagBottle() {
  ...
  try {
    ...
    editingTags.clear();
    inferred.tags.forEach((tag) => editingTags.add(tag));
    renderTagButtons(elements.bottleTags, editingTags, "bottle");
    ...
    setAutoTagStatus(`${source}；推荐 ${inferred.tags.map((tag) => flavorName[tag]).join("、") || "清爽"}`, "good");
  } catch {
...
function generateRecipe() {
  ...
  if (strength >= 4) tags.add("strong");
  if (strength <= 2) tags.delete("strong");
...
function applyFlavorTemplateBias(weight, template, tags) {
  ...
  if (tags.has("fruity")) {
    if (hasTag("fruity") || hasRole("jamOrSyrup") || hasRole("orangeLiqueur")) {
      nextWeight += 6;
    }
  }
  if (tags.has("bright")) {
...
function buildIngredientsFromTemplate({ template, base, modifier, citrus, syrup, mixer, tags, strength }) {
  ...
  template.roles.forEach((part) => {
    if (part.optional && Math.random() > 0.55) return;
...
function renderSaved() {
  ...
  elements.savedList.innerHTML = state.savedRecipes.map((recipe) => `
    <article class="saved-card">
      ...
      <div class="recipe-actions">
        <button class="ghost-button" data-action="restore" data-id="${recipe.id}" type="button">查看</button>
  ...
  elements.savedList.querySelectorAll("[data-action]").forEach((button) => {
    button.addEventListener("click", () => {
      const { action, id } = button.dataset;
      if (action === "restore") {
        currentRecipe = state.savedRecipes.find((recipe) => recipe.id === id);
        renderRecipe(currentRecipe);
        switchView("inspireView");
      }
      if (action === "remove") {
